...
for i in range(nPeticiones):
    peticiones.append(v.validar(input(f'Ingrese la peticion {i + 1}: '),ls=nCilindros))

cilindroInicial = v.validar(input(f'Ingrese el cilindro inicial[0-{nCilindros}]: '),ls=nCilindros)
...
if seleccion == 0:
    ...
    resultados = m.SCAN(nCilindros, peticiones, cilindroInicial)
elif seleccion == 3:
